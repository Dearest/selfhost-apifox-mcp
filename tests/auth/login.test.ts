...
describe('LoginManager', () => {
  const baseUrl = 'https://api.dev.longbridge-inc.com';
  ...
  it('首次获取 token 时调用登录接口并缓存', async () => {
    ...
    mock
      .onPost(`${baseUrl}/api/v1/login`)
      .reply(200, { success: true, data: { accessToken: 'token-1' } });
  ...
  it('forceRefresh=true 时重新登录', async () => {
    ...
    mock
      .onPost(`${baseUrl}/api/v1/login`)
      .replyOnce(200, { success: true, data: { accessToken: 'token-1' } });
    mock
      .onPost(`${baseUrl}/api/v1/login`)
      .replyOnce(200, { success: true, data: { accessToken: 'token-2' } });
